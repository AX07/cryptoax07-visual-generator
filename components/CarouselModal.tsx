  ...
  const generateCarouselImages = async () => {
    ...
    try {
        ...
        const promises = slides.map(async (slide, index) => {
            try {
                // Use the unique prompt generated by AI for this specific slide
                ...
                return { ...slide, imageUrl: url, loading: false, prompt: specificPrompt };
            } catch (e) {
                console.error(e);
                return { ...slide, loading: false }; // Keep error state or old state
            }
        });

        const completedSlides = await Promise.all(promises);
